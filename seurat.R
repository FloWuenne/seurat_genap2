...
option_list <- list(
    make_option(c("-dge_input","--dge_input"), type="character", help="Input file"),
...
ReadAlevin <- function( base.path = "." ){
    ...
    barcode.loc <- paste0( base.path, "/alevin_output/alevin/quants_mat_rows.txt" )
    gene.loc <- paste0( base.path, "/alevin_output/alevin/quants_mat_cols.txt" )
    matrix.loc <- paste0( base.path, "/alevin_output/alevin/quants_mat.csv" )
...
## salmon alevin output
	...
	counts <- ReadAlevin(args$dge_input
}
